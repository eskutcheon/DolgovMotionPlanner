...
class HybridAStarPlannerBase:
    ...
    def __init__(
        ...
        *,
        ...
        self.indexer = Indexer(occ_grid, kappa_bins=config.kappa_bins, kappa_max=config.kappa_max)
        self.model = BicycleModel(config.vehicle)
        # # TODO: replace steering samples with curvature-rates samples (e.g. $u \in \{-\kappa_{max}, 0, +\kappa_{max}\}$)
        #     # then integrate over arc length $\Delta s$
        # # TODO: keep existing n_substeps approach but integrate kappa forward over the substeps
        ...
        if use_rectangle_footprint:
            step = config.footprint_sample_step or 0.5 * float(config.grid.resolution)
        ...
        self._gate_radius_m = rectangle_circumscribed_radius(self.cfg.vehicle) + 0.5 * res * SQRT2
    ...
    def _pose_is_free_fast(self, pose: Pose) -> bool:
        ...
        return pose_is_free(pose, self.map, self.footprint_offsets)


    def _try_goal_shot(self, start_pose: Pose, goal: GoalSpec, h2d: HolonomicWithObstacles2D) -> Optional[List[Pose]]:
        """Fast 'analytic-like' attempt: greedily roll out a short sequence of (σ,u) to reach the goal tolerance."""
        ...
        *,
        ...
        while open_heap and stats.expanded < int(max_expansions):
            ...
            if self._goal_reached(cur.pose, goal):
                stats.end_time_s = time.perf_counter()
                path = self._reconstruct(nodes, nid)
                if len(path) == 0:
            ...
            for direction in directions:
                ...
                for u in self.u_set:
                    res = self.model.rollout(
                        ...
                        dO_m=self._dO,
                        gate_radius_m=self._gate_radius_m,
                        exact_check_margin_m=self._exact_margin_m,
                        footprint_cache=self._footprint_cache,
                        theta_bins=int(self.cfg.grid.theta_bins),
                        rho=self._rho,
                    )
                    ...
                    # if g2 >= best_g[nxt_key.iy, nxt_key.ix, nxt_key.itheta, nxt_key.ikappa]:
                    ...
                    n2 = HybridNode(
                        ...
                        pose=nxt_pose,
                        parent_id=nid,
                        g=float(g2), h=float(h2), f=float(g2 + h2),
                        parent_action=(int(direction), float(u)),
                    )
                    nodes.append(n2)
                    nid2 = len(nodes) - 1
...
class HybridAStarPlannerCpp(HybridAStarPlannerBase):
    ...
    def plan(
        ...
        *,
        ...
        stats = PlannerStats()
        # TODO: if this structure for benchmarking is kept, make a PlannerStatsContext to compute and hold benchmarking (timing, etc) within a context manager
        stats.start_time_s = time.perf_counter()
        # Goal-dependent holonomic-with-obstacles heuristic.
        h2d = self._build_goal_heuristics(goal)
