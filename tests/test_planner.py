...
from utils import pose_is_free, adjust_start_pose_for_clearance
...
#!!! FIXME: throwing errors because the start position is in collision, even though that shouldn't be the case
    # there's either some bad logic in the planner files or the mapping between world to grid state is off at times (possibly because of resolution issues)
@pytest.mark.slow
def test_python_backend_handles_mazes(states_from_maze, planner_config, start_pose, goal_spec):
    grid, start, goal = states_from_maze
    ...
    print("start pose values (grid frame): ", grid.world_to_grid(start[0], start[1]))
    s_pose = Pose(start[0], start[1], start_pose.theta, start_pose.kappa)
    print("goal pose values (world frame): ", goal)
    print("goal pose values (grid frame): ", grid.world_to_grid(goal[0], goal[1]))
    ix, iy = grid.world_to_grid(start[0], start[1])
    print("grid cell at start pose: ", grid.occ[iy, ix])
    print("grid values: \n", grid.occ.astype(np.uint8))
    g_pose = Pose(goal[0], goal[1], goal_spec.pose.theta, goal_spec.pose.kappa)
    ix, iy = grid.world_to_grid(goal[0], goal[1])
    print("grid cell at goal pose: ", grid.occ[iy, ix])
    ...
    #!!! FIXME: this still happens in certain cases like '6x6_square_res30'
    assert stats.expanded > 0, "Planner did not expand any nodes"
    assert len(path) > 1, "Planner failed to find a path"
